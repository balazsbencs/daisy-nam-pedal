...
QspiStorage::Status QspiStorage::Init()
{
    ...
    const size_t directory_end = sizeof(NamDataHeader)
                                 + static_cast<size_t>(header_->count) * sizeof(NamDataEntry);
    const auto* dir = reinterpret_cast<const NamDataEntry*>(header_ + 1);
    for(uint16_t i = 0; i < header_->count; ++i)
    {
        const NamDataEntry& e = dir[i];
        if(e.type > NAM_ENTRY_PRESET
           || memchr(e.name, '\0', NAM_DATA_NAME_LEN) == nullptr
           || e.offset < directory_end
           || e.offset > NAM_DATA_PARTITION_SIZE
           || e.length > NAM_DATA_PARTITION_SIZE - e.offset)
        {
            status_ = Status::BAD_MAGIC;
            return status_;
        }
